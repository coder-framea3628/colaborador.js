...
const appState = {
  ...
  detecting: false,
  faceDetector: null,
  failureCount: parseInt(localStorage.getItem('failureCount')) || 0,
  lastFailureTime: parseInt(localStorage.getItem('lastFailureTime')) || 0,
...
const domElements = {
  ...
  permissionPopup: document.getElementById('permissionPopup'),
  reloadBtn: document.getElementById('reloadBtn'),
  cpfInput: document.getElementById('cpf'),
  phoneInput: document.getElementById('phone'),
  detectionCanvas: (function() {
...
function showToast(message, type = 'info') {
  ...
  domElements.toast.classList.remove('error', 'success');
  if (type === 'error') domElements.toast.classList.add('error');
  if (type === 'success') domElements.toast.classList.add('success');
  domElements.toast.classList.add('active');
  setTimeout(() => domElements.toast.classList.remove('active'), 3000);
}

// Check attempt limit
function checkAttemptLimit() {
  const now = Date.now();
  if (appState.failureCount >= 3 && now - appState.lastFailureTime < 24 * 60 * 60 * 1000) {
    showToast('Muitas tentativas falhas. Tente novamente em 24 horas.', 'error');
    return true;
  }
  // Auto-reset after 24h
  if (now - appState.lastFailureTime >= 24 * 60 * 60 * 1000) {
    resetFailures();
  }
  return false;
}
...
function incrementFailure() {
  appState.failureCount++;
  appState.lastFailureTime = Date.now();
  localStorage.setItem('failureCount', appState.failureCount);
  localStorage.setItem('lastFailureTime', appState.lastFailureTime);
}

// Reset failures
function resetFailures() {
  appState.failureCount = 0;
  localStorage.setItem('failureCount', 0);
}

// Form validation
function validateForm() {
  const cpfInput = domElements.cpfInput;
  const nameInput = document.getElementById('name');
  const cargoInput = document.getElementById('cargo');
  const emailInput = document.getElementById('email');
  const phoneInput = domElements.phoneInput;
  ...
  if (!nameInput.value.trim()) {
    nameInput.setCustomValidity('Nome completo é obrigatório.');
    valid = false;
  } else {
  ...
  if (!cargoInput.value.trim()) {
    cargoInput.setCustomValidity('Cargo é obrigatório.');
    valid = false;
  } else {
  ...
  if (!passwordInput.value.trim()) {
    ...
    passwordInput.setCustomValidity('Senha deve ter no mínimo 8 caracteres.');
    valid = false;
  } else {
  ...
  if (!valid) {
    domElements.registerForm.reportValidity();
  }
...
async function startVerification() {
  ...
  domElements.left.style.display = 'none';
  domElements.container.classList.add('verification');
  domElements.cameraStage.style.display = 'flex';
  domElements.cameraStage.setAttribute('aria-hidden', 'false');
...
async function startCamera() {
  if (location.protocol !== 'https:') {
    showToast('A verificação facial requer HTTPS para acessar a câmera.', 'error');
    return;
  }
  ...
  try {
    ...
    let constraints = {
      audio: false,
      video: {
        facingMode: appState.usingFacingMode,
        width: { ideal: 1280 },
        height: { ideal: 960 }
      }
    };
    ...
    if (appState.currentDeviceId) {
      constraints.video.deviceId = { exact: appState.currentDeviceId };
    }

    appState.stream = await navigator.mediaDevices.getUserMedia(constraints);
    domElements.preview.srcObject = appState.stream;
    await domElements.preview.play();

    const allDevices = await navigator.mediaDevices.enumerateDevices();
    appState.devices = allDevices.filter(d => d.kind === 'videoinput');

    const track = appState.stream.getVideoTracks()[0];
    appState.currentDeviceId = track.getSettings().deviceId || appState.currentDeviceId;

    appState.detecting = true;
    domElements.faceOverlay.style.display = 'flex';
    domElements.captureRow.style.display = 'flex';

    if ('FaceDetector' in window) {
      appState.faceDetector = new window.FaceDetector({ fastMode: true });
      detectFace();
    } else {
      showToast('Detecção facial não disponível. Use o botão para capturar manualmente.', 'info');
      domElements.manualCaptureBtn.style.display = 'inline-block';
    }
  } catch (err) {
    ...
    if (err.name === 'OverconstrainedError') {
      ...
      try {
        ...
        appState.stream = await navigator.mediaDevices.getUserMedia(fallbackConstraints);
        domElements.preview.srcObject = appState.stream;
        await domElements.preview.play();
      } catch (fallbackErr) {
...
function handleCameraError(err) {
  if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
    domElements.permissionPopup.style.display = 'block';
  } else {
...
function stopStream() {
  ...
  if (appState.stream) {
    ...
    domElements.preview.srcObject = null;
  }
}

// Face detection loop (throttled to 500ms)
async function detectFace() {
  if (!appState.detecting || !domElements.preview.videoWidth) return;

  domElements.detectionCanvas.width = domElements.preview.videoWidth;
  domElements.detectionCanvas.height = domElements.preview.videoHeight;
  const ctx = domElements.detectionCanvas.getContext('2d');
  ctx.drawImage(domElements.preview, 0, 0);

  try {
    const detections = await appState.faceDetector.detect(domElements.detectionCanvas);
    if (detections.length > 0) {
      const box = detections[0].boundingBox;
      const centerX = domElements.preview.videoWidth / 2;
      const centerY = domElements.preview.videoHeight / 2;
      const faceCenterX = box.x + box.width / 2;
      const faceCenterY = box.y + box.height / 2;
      const distance = Math.sqrt(Math.pow(faceCenterX - centerX, 2) + Math.pow(faceCenterY - centerY, 2));
      const maxDistance = domElements.preview.videoWidth * 0.15;
      if (distance < maxDistance && box.width > domElements.preview.videoWidth * 0.3) {
        domElements.faceOverlay.classList.add('detected');
        appState.detecting = false;
        clearInterval(appState.detectionInterval);
        startProcessing();
        return;
      } else {
        domElements.faceOverlay.classList.remove('detected');
      }
    } else {
      domElements.faceOverlay.classList.remove('detected');
    }
...
function manualCapture() {
  domElements.manualCaptureBtn.disabled = true;
  startProcessing();
}
...
async function startProcessing() {
  ...
  processingOverlay.style.borderRadius = '12px'; // Minimalist: rect from start
  ...
  domElements.previewWrap.appendChild(processingOverlay);
  domElements.faceOverlay.style.display = 'none';
  domElements.captureRow.style.display = 'none';
...
function showErrorResult(overlay, spinner, text, reason) {
  ...
  back.addEventListener('click', () => {
    back.disabled = true;
    domElements.container.classList.remove('verification');
    domElements.left.style.display = 'block';
    domElements.cameraStage.style.display = 'none';
    stopStream();
...
async function switchCamera() {
  domElements.switchCamBtn.disabled = true;
  appState.usingFacingMode = appState.usingFacingMode === 'user' ? 'environment' : 'user';
  if (appState.devices.length > 1) {
    // Cycle through deviceIds for better support
    const currentIndex = appState.devices.findIndex(d => d.deviceId === appState.currentDeviceId);
    appState.currentDeviceId = appState.devices[(currentIndex + 1) % appState.devices.length].deviceId;
  }
  await startCamera();
  domElements.switchCamBtn.disabled = false;
}
...
function cleanup() {
  ...
  domElements.startBtn.removeEventListener('click', startVerification);
  domElements.manualCaptureBtn.removeEventListener('click', manualCapture);
  domElements.switchCamBtn.removeEventListener('click', switchCamera);
  domElements.reloadBtn.removeEventListener('click', reloadPage);
  domElements.registerForm.removeEventListener('submit', handleFormSubmit);
...
domElements.startBtn.addEventListener('click', startVerification);
domElements.switchCamBtn.addEventListener('click', switchCamera);
domElements.manualCaptureBtn.addEventListener('click', manualCapture);
domElements.reloadBtn.addEventListener('click', reloadPage);
domElements.registerForm.addEventListener('submit', handleFormSubmit);
window.addEventListener('beforeunload', cleanup);

// CPF mask
domElements.cpfInput.addEventListener('input', (e) => {
...
// Phone mask
domElements.phoneInput.addEventListener('input', (e) => {
  let val = e.target.value.replace(/\D/g, '');
...
async function detectFaceLoop() {
  // The detection logic moved here from detectFace
  if (!appState.detecting || !domElements.preview.videoWidth) return;
